...
export function PedagioForm({ route, navigation }: PedagioFormProps) {
  ...
  const { valores, adicionarLinhaVazia, salvarLinha, removerLinha } = usePedagioValores(mode,pedagioId);
  const colunas: { key: keyof PedagioValor; label: string; keyboardType: "numeric" | "default" }[] = [
    { key: 'pedagioValorNumeroEixos', label: 'Número de eixos', keyboardType: 'numeric' },
  { key: 'pedagioValorPedagio', label: 'Valor (R$)', keyboardType: 'numeric' },
